...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Allow all origins (temporary fix for debugging)
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>Need help? Reply to this email or visit <strong>airgo.com</strong></p>
                </div>
...
async def chat(chat_message: ChatMessage):
    try:
        ...
        if any(keyword in message for keyword in ["flight", "book", "fly", "lagos", "abuja", "port harcourt", "kano", "enugu"]):
            origin = None
            destination = None
            ...
            if origin and destination:
                try:
                    ...
                    if amadeus_response:
                        parsed = amadeus.parse_flight_data(amadeus_response)
                        if parsed:
                            flight_data = parsed
                except Exception as flight_err:
                    print(f"Flight search error (non-fatal): {flight_err}")
                    # Continue without flight data - Claude will still respond gracefully
        ...
        import traceback
        traceback.print_exc()
        # Never show raw errors to users - try to still give a helpful response
        try:
            fallback_response = await process_with_claude(
                chat_message.message,
                flight_data=None,
                conversation_history=chat_message.history
            )
            ...
            return {"response": "I'm having a brief connection issue. Please try your request again in a moment!"}
