...
async def chat(chat_message: ChatMessage):
    try:
        ...
        if any(keyword in message for keyword in ["flight", "book", "fly", "lagos", "abuja", "port harcourt", "kano", "enugu"]):
            ...
            if origin and destination:
                ...
                if amadeus_response:
                    ...
                    if not flight_data:
                        print(f"WARNING: Amadeus returned data but parsing yielded 0 flights. Raw keys: {list(amadeus_response.keys()) if isinstance(amadeus_response, dict) else 'not a dict'}")
        ...
        import traceback
        traceback.print_exc()
        return {"response": "Something went wrong on our end. Please try again — if you were searching for a flight, try rephrasing your request (e.g. 'Lagos to Abuja on Saturday')."}
